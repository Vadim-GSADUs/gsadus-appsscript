...
var GS = (function (GS: any = {}) {
  ...
  GS._findHeader = function(sheet: GoogleAppsScript.Spreadsheet.Sheet, headerName: string, maxScan = 50) {
  ...
  GS.Path = GS.Path || {};
  GS.Path.id = function(rel: string, refresh?: boolean): string {
  ...
  GS.Path.folder = function(rel: string, refresh?: boolean): GoogleAppsScript.Drive.Folder {
  ...
  GS.Path.writeDiagnostics = function() {
    ...
    Object.keys(paths).sort().forEach(key => {
      const rel = String((paths as Record<string, string>)[key] || '');
  ...
  function pickLatestCsvLikeInFolder_(folderId: string, basename: string): { id: string; name: string; updated: GoogleAppsScript.Base.Date } | null {
  ...
  function fetchCsvText_(info: { id: string; name: string }): string {
    const blob = DriveApp.getFileById(info.id).getBlob();
    return stripBom_(blob.getDataAsString());
  }

  function stripBom_(s: string): string {
    if (!s) return s;
  ...
  const idx = (name: string): number => {
    ...
    const models = [];
  const perCol = new Map<string, any[]>();
  ...
  function tcol(name: string): number {
      const n = norm(name);
  ...
  if (nrows) cat.getRange(hr+2, colIdx, nrows, 1).setValues((perCol.get(name) as any[]).map((v: any)=>[v]));
    });
  };
  ...
  function ensureTargetColumn(name: string): number {
      const n = norm(name);
      for (let c=0; c<targetHeader.length; c++) if (norm(String(targetHeader[c]||'')) === n) return c+1;
      const colIndex = targetHeader.length + 1;
      cat.getRange(hr+1, colIndex).setValue(name);
  ...
  function lookupByModel_(sheetName: string, valueHeader?: string) {
...
GS.Registry.refresh = function() {
  ...
  (function walk(folder: GoogleAppsScript.Drive.Folder, parts: string[]) {
  ...
  copyValuesOnly_(src, dst, CFG.CATALOG_TAB);      // publishes Catalog (active configured tab)
    ...
    function copyValuesOnly_(
      srcSS: GoogleAppsScript.Spreadsheet.Spreadsheet,
      dstSS: GoogleAppsScript.Spreadsheet.Spreadsheet,
      tabName: string
    ) {
  ...
  GS.ConfigHelper._findTable_ = function(tableName: string) {
    const ss = SpreadsheetApp.getActive();
    try {
      // Avoid restrictive fields mask; some deployments may not expose 'tables' unless full object is returned
      const info: any = Sheets.Spreadsheets.get(ss.getId());
  ...
  GS.ConfigHelper._writeTable_ = function(tableName: string, header: string[], rows: any[][], defaultAnchor?: { row: number; col: number }) {
    const ss = SpreadsheetApp.getActive();
    ...
    if (dataRowCount > 0) {
      ...
      for (let i = area.length - 1; i >= 0; i--) {
        const row = area[i];
  if (row.some((c: unknown) => String(c).trim() !== '')) { lastNonEmpty = i; break; }
      }
      if (lastNonEmpty >= 0) {
  ...
  GS.ConfigHelper._notify_ = function(msg: string) {
    GS.ConfigHelper._notifications = GS.ConfigHelper._notifications || [];
    GS.ConfigHelper._notifications.push(String(msg));
  };

  GS.ConfigHelper._writeTableStrict_ = function(tableName: string, header: string[], rows: any[][]) {
    const found = GS.ConfigHelper._findTable_ ? GS.ConfigHelper._findTable_(tableName) : null;
    if (!found) {
  ...
  // Avoid restrictive fields mask to ensure 'tables' is present on all deployments
    ...
    try {
  const spreadsheetInfo: any = Sheets.Spreadsheets.get(spreadsheetId);
      if (spreadsheetInfo.sheets) {
        for (const sheetInfo of spreadsheetInfo.sheets) {
          ...
          const s = ss.getSheetByName(sheetInfo.properties.title as string);
          if (!s) continue; // Skip if sheet isn't found

          for (const table of sheetInfo.tables as any[]) {
            const tableName = table.name as string;
            const r = table.range as any;
            if (!r) continue;
            ...
            const tabName = sheetInfo.properties.title as string;
